...
const chipData = [
  {
    ...
    value: 'React · Node',
    position: { top: '-32px', right: '32%' },
    delay: '-2s',
  },
  {
    label: 'Colaboración',
    value: 'Remota · Global',
    position: { bottom: '-28px', left: '18%' },
    delay: '-4s',
  },
  {
    label: 'Enfoque',
    value: 'Diseño + Código',
    position: { top: '38%', right: '-52px' },
    delay: '-6s',
  },
];

const HoloRing = styled.div`
  position: absolute;
  left: 50%;
  top: 50%;
  width: min(520px, 88%);
  ...
  box-shadow: 0 0 0 60px rgba(56, 249, 215, 0.02);
  opacity: 0.6;
...
const AccentLight = styled.div`
  position: absolute;
  inset: 32px;
  border-radius: 36px;
  background: radial-gradient(circle at 30% 30%, rgba(56, 249, 215, 0.18), transparent 70%),
    radial-gradient(circle at 70% 70%, rgba(139, 92, 246, 0.22), transparent 75%);
  filter: blur(60px);
  opacity: 0.6;
  mix-blend-mode: screen;
...
const FloatingChip = styled.div`
  ...
  border: 1px solid rgba(255, 255, 255, 0.12);
  box-shadow: 0 18px 40px rgba(6, 6, 12, 0.55);
  ...
  &:before {
    ...
    height: 2px;
    border-radius: 999px;
    background: linear-gradient(90deg, rgba(56, 249, 215, 0.8), rgba(139, 92, 246, 0.2));
  }
...
const FloatingChipLabel = styled.span`
  font-size: 0.74rem;
  letter-spacing: 0.18em;
  text-transform: uppercase;
  color: rgba(233, 233, 255, 0.56);
`;

const FloatingChipValue = styled.span`
  font-size: 0.98rem;
  font-weight: 600;
  letter-spacing: 0.06em;
  color: #ffffff;
...
const Hero = () => {
  return (
    <HeroSection id="top">
      ...
      <HeroVisual>
        <HoloRing aria-hidden />
        <Blob aria-hidden />
        <AccentLight aria-hidden />
        <Card>
        ...
        {chipData.map((chip) => (
          <FloatingChip key={chip.label} {...chip.position} delay={chip.delay}>
            <FloatingChipLabel>{chip.label}</FloatingChipLabel>
            <FloatingChipValue>{chip.value}</FloatingChipValue>
          </FloatingChip>
        ))}
        <Orbit aria-hidden>
